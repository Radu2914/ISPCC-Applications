...
Usage:
    python phosphorus.py                             # landmark elements
    python phosphorus.py --all                       # all elements + TSA fingerprint
    python phosphorus.py --query Fe                  # nearest 5 to iron in TSA space
...
# ── ENCODING FUNCTIONS v2 ─────────────────────────────────────────────────────
# Compounding symbolic encoding. π and e appear as exponents of x, not as
# scalar multipliers. Terms are not separable by linear methods — the
# "weight" of each term is determined by x itself through the exponent
# interaction. Normalisers are irrational combinations of π and e.

def phi_pi(x: float) -> float:
    """
    Π-basis: cascade / non-returning character.
    Five compounding terms on x^(1/e), x^(1/π), x^(e/π), x^(π/e).
    Normaliser: π + e − 1  (irrational, not a count of terms).
    """
    u = max(1e-15, min(float(x), 10.0))
    # irrational exponent transforms — each produces a distinct non-periodic basis
    u_ie  = u ** (1 / E)           # x^(1/e)  ≈ x^0.368
    u_ipi = u ** (1 / PI)          # x^(1/π)  ≈ x^0.318
    u_epi = u ** (E / PI)          # x^(e/π)  ≈ x^0.865
    u_pie = u ** (PI / E)          # x^(π/e)  ≈ x^1.156
    t1 = math.sin(PI  * u_ie)
    t2 = math.cos(E   * u_ipi)
    t3 = t1 * math.exp(-u_ipi / E)            # t1 compounded with irrational decay
    t4 = math.sin(PI**2 * u_epi)
    t5 = t2 * math.sin(E * u_pie)             # t2 compounded with orthogonal oscillation
    return (t1 + t2 + t3 + t4 + t5) / (PI + E - 1.0)

def phi_e(x: float) -> float:
    """
    Ε-basis: equilibrium / bounded character.
    Three compounding terms on x^(π/e), x^(e/π), x^π.
    Gaussian centred at 1/e — the Ε-attractor of the unit interval.
    Normaliser: e.
    """
    u = max(1e-15, min(float(x), 1.0))
    t1 = math.exp(-E  * u ** (PI / E))              # decay with irrational exponent
    t2 = u ** (E / PI) * math.exp(-u**PI / PI)      # bell: power rise × irrational decay
    t3 = math.exp(-(E / PI) * (u - 1.0 / E) ** 2)  # Gaussian centred at 1/e ≈ 0.368
    return (t1 + t2 + t3) / E

def phi_b(x: float) -> float:
    """
    Β-basis: boundary / separatrix character.
    Three compounding Gaussians on v = (x − 0.5)² with exponents 1/e and 1/π.
    Peaked at x = 0.5 (the separatrix). Normaliser: π + e.
    """
    u  = max(0.0, min(float(x), 1.0))
    v  = (u - 0.5) ** 2 + 1e-15               # always ≥ 0; epsilon guards 0^irrational
    t1 = math.exp(-PI       * v ** (1.0 / E))  # Gaussian in v^(1/e) space
    t2 = math.exp(-E        * v ** (1.0 / PI)) # Gaussian in v^(1/π) space
    t3 = math.exp(-(PI**2 / E) * v)            # Gaussian with symbolic width π²/e
    return (t1 + t2 + t3) / (PI + E)
