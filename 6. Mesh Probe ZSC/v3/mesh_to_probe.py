...
# ═══════════════════════════════════════════════════════════════════════════════
# EDGE EXTRACTION — zone boundary edges (the Β entities)
# Where two face zones meet. Each unique zone pair = one boundary edge entry.
# ═══════════════════════════════════════════════════════════════════════════════

def extract_zone_edges(verts_mm, zone_ids_arr, normals_n):
    """
    Extract all edges shared between triangles of different zones.
    One row per unique zone pair — aggregates dihedral, length, centroid.

    Returns list of dicts: edge_id, zone_a, zone_b, dihedral_deg,
    boundary_length, mid_x, mid_y, mid_z
    """
    def round_v(v, decimals=4):
        return tuple(np.round(v, decimals))

    edge_to_tris = {}
    for i, (tri, z) in enumerate(zip(verts_mm, zone_ids_arr)):
        for e in range(3):
            va  = round_v(tri[e])
            vb  = round_v(tri[(e + 1) % 3])
            key = tuple(sorted([va, vb]))
            edge_to_tris.setdefault(key, []).append((i, int(z)))

    zone_pair_data = {}
    for key, tri_list in edge_to_tris.items():
        if len(tri_list) != 2:
            continue
        (i0, z0), (i1, z1) = tri_list
        if z0 == z1:
            continue

        za, zb   = min(z0, z1), max(z0, z1)
        n0, n1   = normals_n[i0], normals_n[i1]
        dot      = np.clip(np.dot(n0, n1), -1.0, 1.0)
        dihedral = float(np.degrees(np.arccos(dot)))
        va_arr   = np.array(key[0])
        vb_arr   = np.array(key[1])
        midpoint = (va_arr + vb_arr) / 2.0
        length   = float(np.linalg.norm(va_arr - vb_arr))

        zone_pair_data.setdefault((za, zb), []).append(
            (dihedral, midpoint, length))

    edges = []
    for edge_id, ((za, zb), data) in enumerate(sorted(zone_pair_data.items())):
        dihedrals = [d[0] for d in data]
        midpoints = np.array([d[1] for d in data])
        ...
        edges.append({
            ...
            'zone_a':          za + 1,    # 1-based
            'zone_b':          zb + 1,    # 1-based
            'dihedral_deg':    round(float(np.mean(dihedrals)), 2),
            'boundary_length': round(float(np.sum(lengths)), 4),
            'mid_x':           round(float(centroid[0]), 4),
            'mid_y':           round(float(centroid[1]), 4),
            'mid_z':           round(float(centroid[2]), 4),
        })
...
# ═══════════════════════════════════════════════════════════════════════════════
# CORNER EXTRACTION — vertices shared by 3+ zones (extreme Β)
# ═══════════════════════════════════════════════════════════════════════════════

def extract_corner_points(verts_mm, zone_ids_arr):
    """
    Extract vertices shared by 3 or more different zones.
    Returns list of dicts: corner_id, x, y, z, adjacent_zones
    """
    def round_v(v, decimals=4):
        return tuple(np.round(v, decimals))

    vertex_zones = {}
    for tri, z in zip(verts_mm, zone_ids_arr):
        for v in tri:
            key = round_v(v)
            vertex_zones.setdefault(key, set()).add(int(z))

    corners   = []
    corner_id = 1
    for key, zones in sorted(vertex_zones.items()):
        if len(zones) >= 3:
            corners.append({
                'corner_id':      corner_id,
                'x':              round(key[0], 4),
                'y':              round(key[1], 4),
                'z':              round(key[2], 4),
                'adjacent_zones': ','.join(str(z + 1) for z in sorted(zones)),
            })
            corner_id += 1
...
def find_zones_for_nodes(node_ids, nodes_df, verts_mm, zone_ids_arr, tol=0.01):
    ...
    those nodes most belong to.

    Corner and edge nodes appear as vertices of triangles from multiple
    zones. Rather than assigning them to whichever triangle the KD-tree
    returns first, we find ALL zones that share that vertex and assign
    the node to the zone whose centroid is nearest. This ensures corner
    and edge nodes are attributed to the face they sit most centrally on,
    not to an adjacent face that happens to share the corner.
    ...
    Returns:
        ...
        unmatched     : list of int — node IDs with no matching STL vertex
    """
    n_zones   = int(zone_ids_arr.max()) + 1
    all_verts = verts_mm.reshape(-1, 3)               # (N_tri×3, 3)
    tri_idx   = np.repeat(np.arange(len(verts_mm)), 3)
    tree      = cKDTree(all_verts)

    # Pre-compute zone centroids (area-weighted)
    zone_centroids = np.zeros((n_zones, 3))
    for z in range(n_zones):
        mask = (zone_ids_arr == z)
        tris = verts_mm[mask]                          # (k, 3, 3)
        if len(tris) > 0:
            zone_centroids[z] = tris.reshape(-1, 3).mean(axis=0)
    ...
    for nid in node_ids:
        ...
        # Find ALL STL vertices within tolerance (not just the nearest one)
        indices = tree.query_ball_point(coord, r=tol)

        if not indices:
            print(f'  [WARN] Node {nid} at {coord} has no STL vertex within '
                  f'{tol}mm. Node may be interior (not on surface) — skipped.')
            unmatched.append(nid)
            continue

        # Collect all zones that share this vertex
        candidate_zones = set(int(zone_ids_arr[tri_idx[i]]) for i in indices)

        if len(candidate_zones) == 1:
            # Unambiguous — node is interior to one face
            zone_1based = list(candidate_zones)[0] + 1
        else:
            # Corner or edge node — assign to zone whose centroid is nearest
            best_z   = min(candidate_zones,
                           key=lambda z: np.linalg.norm(coord - zone_centroids[z]))
            zone_1based = best_z + 1

        matched_zones.add(zone_1based)
        note = '' if len(candidate_zones) == 1 else f'  (edge/corner node, nearest face wins)'
        print(f'  Node {nid:>4} at ({coord[0]:+.2f}, {coord[1]:+.2f}, {coord[2]:+.2f}) '
              f'→ zone {zone_1based}{note}')
...
def main():
    ...
    edges     = extract_zone_edges(verts_mm, zone_ids, normals_n)
    edge_path = args.out.replace('.csv', '_edges.csv')
    pd.DataFrame(edges).to_csv(edge_path, index=False)
    print(f'  Zone boundary edges: {len(edges)}')
    for e in edges:
        print(f'    Edge {e["edge_id"]:>2}: Zone {e["zone_a"]} ↔ Zone {e["zone_b"]}  '
              f'dihedral={e["dihedral_deg"]:.1f}°  '
              f'length={e["boundary_length"]:.2f}mm')

    # ── Extract and save corners ────────────────────────────────────
    print(f'\n  Extracting corner points...')
    corners     = extract_corner_points(verts_mm, zone_ids)
    corner_path = args.out.replace('.csv', '_corners.csv')
    pd.DataFrame(corners).to_csv(corner_path, index=False)
    print(f'  Corner points (3+ zones): {len(corners)}')

    print(f'\n  Saved:')
    print(f'    {args.out}         (face zones)')
    print(f'    {edge_path}  (zone boundary edges)')
    print(f'    {corner_path}  (corner points)')
    print(f'\n  Next step:')
