...
# Analogous to λ_free, λ_rubber in EM: intrinsic structural scales of the
# system, not derived from data statistics.
#
# δ = 4.6692...: ratio of successive bifurcation intervals in r
#   → governs how fast period-doubling accelerates; natural scale for n_occ
# α = 2.5029...: scaling factor of attractor width between bifurcations
#   → governs how fast the attractor shrinks; natural scale for std, spread
#
# Each normalization below mirrors the Buckingham Pi step in EM:
#   sin(π · gap/λ_free)       →  sin(π · r/R_BIFURCATION)
#   exp(-e · layer/λ_rubber)  →  exp(-e · ac1/SCALE_AC)
# The argument to every trig/exponential function is now structurally
# meaningful rather than dataset-dependent.
# ══════════════════════════════════════════════════════════════════════════════

DELTA = 4.669201609   # Feigenbaum delta — bifurcation interval ratio
ALPHA = 2.502907875   # Feigenbaum alpha — attractor width scaling

# Pi-encoded variable scales (cascading, non-periodic)
SCALE_R        = R_BIFURCATION          # r / R_BIF: fraction of bifurcation threshold
SCALE_DIST     = R_MAX - R_BIFURCATION  # |r-R_BIF| / chaotic regime width ≈ 0.4301
SCALE_STD      = 1.0 / (2.0 * ALPHA)   # attractor width unit ≈ 0.1997
SCALE_VAR      = SCALE_STD ** 2        # variance unit ≈ 0.0399  (std²)
SCALE_SPREAD   = ALPHA                 # spread scales with α ≈ 2.5029
SCALE_N_OCC    = DELTA ** 2           # period count unit ≈ 21.80

# E-encoded variable scales (self-regulating, periodic)
SCALE_AC       = 1.0                                      # ac1/ac2 naturally bounded [−1,1]
...
def encode_pi_func(x, prefix, scale=None, weights=(5, 1, 1, 3, 1)):
    ...
    scale: Feigenbaum-derived normalization constant for this variable.
           When provided, xn = x/scale (structurally grounded).
           When None, falls back to xn = x/x.max() (data-relative, arbitrary).
    """
    x  = np.clip(x, 0, 10)
    if scale is not None:
        xn = x / (scale + EPS)   # Feigenbaum-grounded: argument has structural meaning
    else:
        xn = x / (x.max() + EPS) # fallback: data-relative (arbitrary)
...
def build_encoded_features(stats_df):
    ...
    for key, col, scale in [
        # variable        raw array                          Feigenbaum scale        rationale
        ("std",      stats_df["std_x"].values,    SCALE_STD),      # std / (1/2α): attractor width units
        ("var_d1",   stats_df["var_d1"].values,   SCALE_VAR),      # var / (1/2α)²: variance units
        ("var_d2",   stats_df["var_d2"].values,   SCALE_VAR),      # var / (1/2α)²: variance units
        ("spread",   stats_df["spread"].values,   SCALE_SPREAD),   # spread / α: Feigenbaum width scale
        ("n_occ",    stats_df["n_occ"].values,    SCALE_N_OCC),    # n_occ / δ²: period count unit
        ("r_val",    stats_df["r"].values,        SCALE_R),        # r / R_BIF: fraction of chaos threshold
        ("abs_dist", stats_df["abs_dist"].values, SCALE_DIST),     # |r-R_BIF| / chaotic regime width
    ]:
        pi_enc.update(encode_pi_func(col, f"pienc_{key}", scale=scale))

    e_enc = {}
    inv_n_occ = 1.0 / (stats_df["n_occ"].values + EPS)   # self-regulation variable
    for key, col, scale in [
        # variable        raw array                          Feigenbaum scale        rationale
        ("ac1",       stats_df["ac1"].values,  SCALE_AC),       # ac1 / 1.0: naturally bounded [−1,1]
        ("ac2",       stats_df["ac2"].values,  SCALE_AC),       # ac2 / 1.0: naturally bounded [−1,1]
        ("mean_x",    stats_df["mean_x"].values, SCALE_MEAN_X), # mean / fixed-pt: convergence to attractor
        ("inv_n_occ", inv_n_occ,               SCALE_INV_NOCC), # inv_n_occ / (1/δ): inverse period unit
    ]:
        e_enc.update(encode_e_func(col, f"eenc_{key}", scale=scale))
...
def main():
    ...
    print(f"\n[INFO] Feigenbaum normalization constants (structurally grounded):")
    print(f"  δ  (bifurcation interval ratio) : {DELTA:.9f}")
    print(f"  α  (attractor width scaling)    : {ALPHA:.9f}")
    print(f"  ── Pi-encoded scales (cascading variables) ──────────────────")
    print(f"  r_val    ÷ R_BIFURCATION        : {SCALE_R:.7f}  (chaos threshold)")
    print(f"  abs_dist ÷ chaotic regime width : {SCALE_DIST:.7f}  (R_MAX - R_BIF)")
    print(f"  std_x    ÷ 1/(2α)              : {SCALE_STD:.7f}  (attractor width unit)")
    print(f"  var_d*   ÷ (1/(2α))²           : {SCALE_VAR:.7f}  (variance unit)")
    print(f"  spread   ÷ α                   : {SCALE_SPREAD:.7f}  (Feigenbaum width scale)")
    print(f"  n_occ    ÷ δ²                  : {SCALE_N_OCC:.7f}  (period count unit)")
    print(f"  ── E-encoded scales (self-regulating variables) ─────────────")
    print(f"  ac1/ac2  ÷ 1.0                 : {SCALE_AC:.7f}  (naturally bounded)")
    print(f"  mean_x   ÷ (R_BIF-1)/R_BIF    : {SCALE_MEAN_X:.7f}  (fixed-point at bifurcation)")
    print(f"  inv_n_occ÷ 1/δ                : {SCALE_INV_NOCC:.7f}  (inverse period unit)")
    ...
    if "XGB raw (baseline)" in timing and "XGB full encoded" in timing:
        ...
        print(f"  XGB encoding overhead     : {speedup:.2f}x  (raw vs Feigenbaum-encoded)")
