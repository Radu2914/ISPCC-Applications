...
N_VALUES = [50, 100, 150, 200, 300, 500]
N_SEEDS  = 10
...
def main():
    ...
    X_full_pool  = full_df_pool[feat_cols].values

    print(f"[INFO] raw={X_raw_pool.shape[1]} features  encoded={X_full_pool.shape[1]} features")
    ...
    # ── Console summary ──────────────────────────────────────────────────────
    print(f"\n{'='*72}")
    print(f"  SAMPLE SIZE SWEEP — XGB encoded vs baselines  ({N_SEEDS} seeds, 5-fold CV)")
    print(f"{'='*72}")
