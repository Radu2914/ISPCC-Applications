...
def encode_pi_func(x, prefix, scale=None, weights=(5, 1, 1, 3, 1)):
    ...
    scale: physical normalizing constant (Buckingham Pi grounded).
           When provided → xn = x/scale (structurally meaningful argument).
...
# ══════════════════════════════════════════════════════════════════════════════
# RAW INPUT ENCODING — SAME INFORMATION AS BASELINE, DIFFERENT COORDINATES
#
# Encodes only the 4 raw inputs using pi/e basis with Buckingham Pi
# normalization. No Pi groups, no engineered features, no domain knowledge
# beyond variable character classification.
#
# The comparison RF raw (baseline) vs RF raw encoded isolates coordinate
# system quality from feature engineering — identical information content,
# different mathematical representation.
#
# Variable classification:
#   gap          → Pi-type: cascading resonance, non-periodic interference
#                  Normalize by λ_free = 10.714mm → sin(π·gap/λ_free)
#                  encodes "how many resonance cycles fit in the gap"
#
#   upper/lower  → E-type: exponential shielding attenuation
#                  Normalize by λ_rubber = 5.051mm → exp(-e·layer/λ_rubber)
#                  encodes "how many penetration depths is this layer"
#
#   tan_delta    → Pi-type: regime competition, straddles bifurcation
#                  Normalize by DIEL_BIFURCATION = 0.107 → sin(π·tan_d/tan_d_bif)
#                  encodes "how far past the bifurcation point"
#
# Feature count: 5 + 3 + 3 + 5 + 3 cross = 19 features from 4 inputs
# Compare: RF raw baseline has 4 features from same 4 inputs
# ══════════════════════════════════════════════════════════════════════════════

def build_raw_encoded_features(df):
    g   = df["gap"].values
    upl = df["upper_protective_layer"].values
    low = df["lower_protective_layer"].values
    td  = df["protective_layer_dielectric"].values
    ...
    # Gap: Pi-encoded, Buckingham Pi scale = λ_free
    enc.update(encode_pi_func(g,   "rawenc_gap",   scale=LAMBDA_FREE))

    # Upper layer: E-encoded, Buckingham Pi scale = λ_rubber
    enc.update(encode_e_func(upl,  "rawenc_upper", scale=LAMBDA_RUBBER))

    # Lower layer: E-encoded, Buckingham Pi scale = λ_rubber
    enc.update(encode_e_func(low,  "rawenc_lower", scale=LAMBDA_RUBBER))

    # Loss tangent: Pi-encoded, Buckingham Pi scale = bifurcation point
    enc.update(encode_pi_func(td,  "rawenc_tand",  scale=DIEL_BIFURCATION))

    # Cross-products: Pi×E interactions — cascade resonance × shielding decay
    # gap/λ_free × upper/λ_rubber: the fundamental shielding-resonance interaction
    gap_n = np.clip(g   / (LAMBDA_FREE      + EPS), 0, 5)
    upl_n = np.clip(upl / (LAMBDA_RUBBER    + EPS), 0, 2)
    low_n = np.clip(low / (LAMBDA_RUBBER    + EPS), 0, 2)
    td_n  = np.clip(td  / (DIEL_BIFURCATION + EPS), 0, 2)

    enc["rawenc_cross_gap_x_upper"] = (np.sin(PI * gap_n) *
                                        np.exp(-E * upl_n))
    enc["rawenc_cross_gap_x_lower"] = (np.sin(PI * gap_n) *
                                        np.exp(-E * low_n))
    enc["rawenc_cross_tand_x_upper"]= (np.sin(PI * td_n)  *
                                        np.exp(-E * upl_n))
...
def main():
    ...
    X_raw_enc_df = build_raw_encoded_features(df)
    X_raw_enc    = X_raw_enc_df.values
    raw_enc_cols = X_raw_enc_df.columns.tolist()

    print(f"\n[INFO] Feature sets:")
    print(f"  Raw inputs        : {X_raw.shape[1]}")
    print(f"  Raw encoded       : {X_raw_enc.shape[1]}  "
          f"(same 4 inputs, Buckingham Pi basis)")
    print(f"  Pi groups only    : {X_pi.shape[1]}")
    print(f"  Full encoded      : {X_full.shape[1]}")

    # ── Learning curve: N ∈ {50,100,150,200} × 10 independent seeds ───────────
    # Each (N, seed) pair is an independent random subsample — no nesting.
    # rng.choice with a different seed each time gives unrelated draws, so the
    # averaged R² reflects true data-efficiency, not subsample luck.
    # KFold also uses random_state=seed so the CV split is independently
    # randomised per run, not locked to a single partition.
    ...
    CAND_NAMES = [
        "RF raw (baseline)",
        "RF raw encoded",        # same 4 inputs, pi/e basis, Buckingham Pi norm
        "XGB raw encoded",       # same 4 inputs, XGB
        "RF full encoded",
        "XGB full encoded",
    ]
    # lc_results[candidate_name][N] = [r2_seed0, r2_seed1, ...]
    lc_results = {name: {n: [] for n in N_VALUES} for name in CAND_NAMES}

    best_r2, best_name = -999, None
    best_actual, best_pred = None, None
    best_model_obj, best_X, best_y_log = None, None, None

    for N in N_VALUES:
        print(f"\n{'─'*65}")
        print(f"  N = {N}   ({N_SEEDS} independent seeds)")
        print(f"{'─'*65}")
        ...
        for seed in range(N_SEEDS):
            ...
            # Independent CV partition for this seed
            ...
            candidates = {
                "RF raw (baseline)" : (RandomForestRegressor(n_estimators=500,
                                       ...
                                       X_raw[idx]),
                "RF raw encoded"    : (RandomForestRegressor(n_estimators=500,
                                        max_features="sqrt", min_samples_leaf=2,
                                        random_state=42, n_jobs=-1),
                                       X_raw_enc[idx]),
                "XGB raw encoded"   : (xgb.XGBRegressor(n_estimators=300,
            ...
            for name, (model, X_use) in candidates.items():
                ...
                for train_idx, test_idx in kf.split(X_use):
                    model.fit(X_use[train_idx], y_log_N[train_idx])
                ...
                lc_results[name][N].append(r2_m)

                if N == 200 and r2_m > best_r2:
                    best_r2        = r2_m
                    best_name      = name
            ...
            # Progress: one line per seed
            r2_raw     = lc_results["RF raw (baseline)"][N][-1]
            r2_rawenc  = lc_results["RF raw encoded"][N][-1]
            r2_fullenc = lc_results["RF full encoded"][N][-1]
            print(f"  seed={seed}  R²_raw={r2_raw:.4f}  "
                  f"R²_rawenc={r2_rawenc:.4f}  Δ={r2_rawenc-r2_raw:+.4f}  "
                  f"R²_fullenc={r2_fullenc:.4f}")

        # Summary across seeds for this N
        print(f"\n  Averages over {N_SEEDS} seeds:")
        print(f"  {'candidate':<24}  {'mean R²':>8}  {'±std':>6}")
        print(f"  {'─'*42}")
        for name in CAND_NAMES:
            vals = lc_results[name][N]
            print(f"  {name:<24}  {np.mean(vals):>8.4f}  {np.std(vals):>6.4f}")

    print(f"\n{'='*65}")
    print(f"  BEST MODEL (N=200) : {best_name}")
    ...
    # Plot 1: R² vs N — mean ± 1 std across seeds
    ax = axes[0]
    colors_lc  = {
        "RF raw (baseline)" : "#4c9be8",
        "RF raw encoded"    : "#e84c9b",
        "XGB raw encoded"   : "#9b4ce8",
        "RF full encoded"   : "#f5a623",
        "XGB full encoded"  : "#d62728",
    }
    markers_lc = {
        "RF raw (baseline)" : "o",
        "RF raw encoded"    : "P",
    ...
    for name in CAND_NAMES:
        mu  = np.array([np.mean(lc_results[name][n]) for n in N_VALUES])
        std = np.array([np.std( lc_results[name][n]) for n in N_VALUES])
        ax.fill_between(ns, mu - std, mu + std,
                        color=colors_lc[name], alpha=0.15)
        ax.plot(ns, mu, marker=markers_lc[name], linewidth=2, markersize=8,
                color=colors_lc[name], label=name,
                markeredgecolor="white", markeredgewidth=0.6)
    ax.axhline(PREV_BEST, color="gray", linestyle=":", linewidth=1.2, alpha=0.7)
    ax.text(N_VALUES[-1] + 1, PREV_BEST + 0.003, "prev. best",
            color="gray", fontsize=8, va="bottom")
    ax.set_xlabel("Training set size  N")
    ax.set_ylabel(f"R²  (mean ± 1 std,  {N_SEEDS} seeds × 5-fold CV)")
    ax.set_title("R² vs N\nindependent random subsamples")
    ax.set_xticks(N_VALUES)
    ax.legend(fontsize=8, loc="lower right")
    ax.grid(True, alpha=0.3)

    # Plot 2: Actual vs Predicted (best model, N=200)
    axes[1].scatter(best_actual, best_pred, alpha=0.5,
    ...
    # Plot 3: Raw encoded vs raw baseline — the clean comparison
    # Feature importance on raw encoded set (all data)
    rf_raw_enc_imp = RandomForestRegressor(n_estimators=500, max_features="sqrt",
                                           min_samples_leaf=2, random_state=42,
                                           n_jobs=-1)
    rf_raw_enc_imp.fit(X_raw_enc, y_log)
    raw_enc_imps = rf_raw_enc_imp.feature_importances_

    def gimp_rawenc(keywords):
        return sum(imp for col, imp in zip(raw_enc_cols, raw_enc_imps)
                   if any(k in col for k in keywords))

    pi_raw_enc  = gimp_rawenc(["rawenc_gap", "rawenc_tand"])
    e_raw_enc   = gimp_rawenc(["rawenc_upper", "rawenc_lower"])
    cross_raw   = gimp_rawenc(["rawenc_cross"])

    cats   = ["Pi-enc\n(gap+tan_d)", "E-enc\n(upper+lower)", "Cross\nproducts"]
    vals   = [pi_raw_enc, e_raw_enc, cross_raw]
    colors = ["#d62728", "#2ca02c", "#9467bd"]
    axes[2].bar(cats, vals, color=colors, edgecolor="k", linewidth=0.8)
    axes[2].set_ylabel("Feature Importance")
    axes[2].set_title("Raw Encoded Importances\n(4 inputs only — same info as baseline)")
    axes[2].grid(True, alpha=0.3, axis="y")
    for i, v in enumerate(vals):
        axes[2].text(i, v+0.003, f"{v*100:.1f}%",
                     ha="center", fontsize=9, fontweight="bold")

    plt.suptitle(
        f"Dimensionless Surrogate — Raw Input Encoding Comparison\n"
        f"εr=4.5, λ_rubber=5.051mm, GAP_SPREAD=4.594mm, r_phantom=12mm"
        f"  |  Best R²={best_r2:.4f}  |  Raw encoded: 19 features from 4 inputs",
        fontsize=11, fontweight="bold")
    plt.tight_layout()
    plt.savefig("surrogate_rawenc_diagnostic.png",
                dpi=150, bbox_inches="tight")
    plt.show()
    print("[INFO] plot saved to surrogate_rawenc_diagnostic.png")

    # ── Save ───────────────────────────────────────────────────────────────────
    best_model_obj.fit(best_X, best_y_log)
    joblib.dump({
...
"""
Usage:
python encode_surrogate.py --dataset "C:/Users/Radu/Desktop/ml project/v31/last_run_designs.csv"
